...
const FOOD_URL = "http://localhost:3000/api/foods/";
...
const FoodItem = ({
  ...
  const [foodName, setFoodName] = useState(nameFood);
  const handleFoodName = (event) => {
    setFoodName(event.target.value);
  };

  const [foodDescription, setFoodDescription] = useState(descriptionFood);
  const handleFoodDescription = (event) => {
    setFoodDescription(event.target.value);
  };

  const [ingredients, setIngredients] = useState(ingredientsFood);
  const handleIngredients = (event) => {
    setIngredients(event.target.value);
  };

  const [foodPrice, setFoodPrice] = useState(priceFood);
  const handleFoodPrice = (event) => {
    setFoodPrice(event.target.value);
  };

  const [foodExpiration, setFoodExpiration] = useState(expirationFood);
